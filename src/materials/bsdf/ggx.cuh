...
// GGX Smith geometry term G(wo, wi) = G1(wo) * G1(wi)
...
// GGX VNDF sampling (Heitz 2018) - returns microfacet normal in local space
...
// =============================================================================
// GGX Conductor (Metal) BSDF
// =============================================================================

struct GGXConductorBSDF {
    float3 eta;    // Real part of complex IOR
    float3 k;      // Imaginary part (extinction coefficient)
    float alpha;   // Roughness (alpha = roughness^2 is common)

    __host__ __device__ GGXConductorBSDF(const float3& F0, float roughness)
        : alpha(fmaxf(0.001f, roughness * roughness)) {
        // Approximate conductor IOR from F0 (reflectance at normal incidence)
        // This is a simplification - real metals need measured spectral data
        eta = F0;
        k = make_float3(1.0f);
    }
    ...
    // Fresnel for conductors (Schlick approximation with F0 from eta, k)
    __host__ __device__ float3 fresnel(float cos_theta) const {
        // Use Schlick with the "color" as F0
    ...
    __host__ __device__ float3 evaluate(const float3& wo_local, const float3& wi_local) const {
        ...
        if (h.z <= 0.0f) return make_float3(0.0f);
    ...
    __host__ __device__ BSDFSample sample(const float3& wo_local, float u1, float u2) const {
        ...
        // Check that wi is in correct hemisphere
        if (sample.wi.z <= 0.0f) {
            return sample;
        }
...
// =============================================================================
// GGX Dielectric (Glass) BSDF
// =============================================================================

struct GGXDielectricBSDF {
    float eta;     // IOR ratio (outside / inside)
    ...
    __host__ __device__ BSDFSample sample(const float3& wo_local, float u1, float u2, float u3) const {
        ...
        if (u3 < F) {
            ...
            if (!refract(wo_local, h, eta_ratio, refracted)) {
                return sample;  // Total internal reflection should be handled by Fresnel
            }
            sample.wi = refracted;
...
struct MirrorBSDF {
    ...
    __host__ __device__ BSDFSample sample(const float3& wo_local) const {
        ...
        sample.f = reflectance;  // f / pdf = reflectance * delta / delta = reflectance
    ...
    __host__ __device__ BSDFSample sample(const float3& wo_local, float u) const {
        ...
        if (u < F) {
            ...
            if (!refract(-wo_local, n, eta_ratio, refracted)) {
                ...
                sample.wi = refracted;
                sample.f = make_float3(eta_ratio * eta_ratio);  // Solid angle compression
                sample.is_transmission = true;
            }
        }
