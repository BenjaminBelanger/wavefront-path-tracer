...
// =============================================================================
// Spectral Dielectric BSDF
// Uses wavelength-dependent IOR for dispersion effects
// =============================================================================

struct SpectralDielectricBSDF {
    float base_ior;          // Base IOR (at 550nm)
    SellmeierCoeffs coeffs;  // Sellmeier coefficients for dispersion
    ...
    __host__ __device__ SpectralDielectricBSDF(float ior, float roughness = 0.0f)
        ...
        // Default to BK7 glass Sellmeier coefficients
    ...
    __host__ __device__ SpectralDielectricBSDF(const SellmeierCoeffs& c, float roughness = 0.0f)
        ...
        base_ior = coeffs.ior(550.0f);  // Reference wavelength
    }
    ...
    // Sample with wavelength-dependent IOR
    __host__ __device__ SpectralBSDFSample sample(
        ...
        // Use hero wavelength for direction sampling
        ...
        // Sample microfacet normal (or perfect specular for smooth glass)
        float3 h;
        if (alpha < 0.01f) {
            h = n;
        } else {
        ...
        if (do_reflect) {
            // Reflection - same for all wavelengths
            ...
            // All wavelengths reflect the same direction
            for (int i = 0; i < NUM_WAVELENGTHS; i++) {
                ...
                sample.f.L[i] = F_i / F_hero;  // Reweight by actual Fresnel
            }
        } else {
            // Refraction - direction varies by wavelength!
            // For hero wavelength sampling, we use the hero direction
            // and reweight other wavelengths

            float3 refracted;
            if (!refract(wo_local, h, eta_hero, refracted)) {
                // TIR - should have been caught by Fresnel
                return sample;
            }
            sample.wi = refracted;
            ...
            // Compute contribution for each wavelength
            for (int i = 0; i < NUM_WAVELENGTHS; i++) {
                float ior_i = get_ior(wavelengths.lambda[i]);
                float eta_i = entering ? (1.0f / ior_i) : ior_i;
                float F_i = fresnel_dielectric(fabsf(cos_theta_o), eta_i);

                // Check if this wavelength also refracts in similar direction
                float3 refracted_i;
                if (refract(wo_local, h, eta_i, refracted_i)) {
                    // Weight by how well the sampled direction matches
                    float dot_match = dot(normalize(refracted_i), normalize(sample.wi));

                    // Only contribute if directions are similar (prevents fireflies)
                    if (dot_match > 0.99f) {
                        sample.f.L[i] = (1.0f - F_i) * eta_i * eta_i / (1.0f - F_hero);
                    } else {
                        // Direction mismatch - this creates dispersion
                        // For accurate dispersion, would need to trace separate rays
                        sample.f.L[i] = 0.0f;
                    }
                } else {
                    sample.f.L[i] = 0.0f;  // TIR for this wavelength
                }
            }
        }
...
// =============================================================================
// Thin Film Interference (for coated surfaces)
// =============================================================================
...
__host__ __device__ inline float3 thin_film_fresnel(
    ...
    // Compute phase difference due to film
    ...
    float r01 = (1.0f - film_ior) / (1.0f + film_ior);  // Air-film
    float r12 = (film_ior - substrate_ior) / (film_ior + substrate_ior);  // Film-substrate

    // Airy summation (simplified)
