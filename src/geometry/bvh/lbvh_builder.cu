...
namespace lumina
{
        ...
        int count)
    {
        ...
        if (extent.x > 0)
            normalized.x = (centroid.x - offset.x) / extent.x;
        if (extent.y > 0)
            normalized.y = (centroid.y - offset.y) / extent.y;
        if (extent.z > 0)
            normalized.z = (centroid.z - offset.z) / extent.z;
        ...
        int num_leaves)
    {
        ...
        int d = (idx == 0 || (__clz(morton_codes[idx] ^ morton_codes[idx + 1]) >
                              __clz(morton_codes[idx] ^ morton_codes[idx - 1])))
                    ? 1
                    : -1;

        int delta_min = (idx == 0 && d == -1) ? -1 : (idx == num_leaves - 1 && d == 1) ? -1
    ...
    void BVH::build(const Triangle *triangles, int count)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            float3 normalized = make_float3(0.5f);
            if (extent.x > 0)
                normalized.x = (centroids[i].x - offset.x) / extent.x;
            if (extent.y > 0)
                normalized.y = (centroids[i].y - offset.y) / extent.y;
            if (extent.z > 0)
                normalized.z = (centroids[i].z - offset.z) / extent.z;
