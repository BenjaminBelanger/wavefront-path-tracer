...
struct Matrix4x4 {
    ...
    __host__ __device__ static Matrix4x4 look_at(const float3& eye, const float3& target, const float3& up) {
        ...
        Matrix4x4 result;
        result(0, 0) = r.x;  result(0, 1) = r.y;  result(0, 2) = r.z;  result(0, 3) = -dot(r, eye);
        result(1, 0) = u.x;  result(1, 1) = u.y;  result(1, 2) = u.z;  result(1, 3) = -dot(u, eye);
        result(2, 0) = -f.x; result(2, 1) = -f.y; result(2, 2) = -f.z; result(2, 3) = dot(f, eye);
